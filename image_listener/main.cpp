    ...
    void imageCallback(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        if (ids.size() > 0) {
            ...
            for(int i=0; i<ids.size(); i++){
                ...
                if(distance>0.3 && distance<3 && abs(rvecs[i][0])<0.04){ //fagat 30cm ta 3m va ba zaviyeye kam ghaboole
                    ROS_INFO("%f    %f     %f  dist:%f", tvecs[i][0],tvecs[i][1],tvecs[i][2],distance);
